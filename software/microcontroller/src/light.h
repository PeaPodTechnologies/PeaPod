  ...
  class PeaPodModuleLighting : public PeaPodModule {
    ...
    public:
      PeaPodModuleLighting(bool chronoCallbacks = true) : PeaPodModule(PEAPOD_MODULENUM_LIGHTING) {
        ...
        // flag_lighting.addLatchingConditional(true, false, callback_lighting_onoff);
        ...
        pwm_lighting_red.set(FSM::Number(0, true, false));
        pwm_lighting_blue.set(FSM::Number(0, true, false));

        registerVariable(&adc_voltage);
        registerVariable(&pwm_lighting_red);
        registerVariable(&pwm_lighting_blue);
        // registerFlag(&flag_lighting);
      }
