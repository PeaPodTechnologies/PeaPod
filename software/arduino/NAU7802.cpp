...
bool NAU7802::begin(uint8_t addr, bool initialize)
{
  _deviceAddress = addr;

  // Serial.print("Connection attempt 1... ");
  if(!isConnected()){
    // Serial.print("Failed.\nConnection attempt 2... ");
    if(!isConnected()){
      // Serial.print("Failed. Check wiring.");
      return false;
    }
  }
  ...
  if (initialize)
  {
    ...
    // if(!result){Serial.println("Failed.");}else{Serial.print("\n");}
    ...
    // Serial.print("Setting gain (x4)... ");
    result &= setGain(NAU7802_GAIN_4); //Set gain to 4
    // if(!result){Serial.println("Failed.");}else{Serial.print("\n");}

    // Serial.print("Setting sample rate (20Hz)... ");
    result &= setSampleRate(NAU7802_SPS_20); //Set samples per second to 20
    // if(!result){Serial.println("Failed.");}else{Serial.print("\n");}

    // Serial.print("Turning off CLK_CHP... ");
    result &= setRegister(NAU7802_ADC, 0x30); //Turn off CLK_CHP. From 9.1 power on sequencing.
    // if(!result){Serial.println("Failed.");}else{Serial.print("\n");}

    // Serial.print("Enabling channel 2 decoupling cap... ");
...
int32_t NAU7802::getAverage(uint16_t averageAmount)
{
  long total = 0;
  uint16_t samplesAquired = 0;
...
void NAU7802::calculateZeroOffset(uint16_t averageAmount)
{
...
void NAU7802::setZeroOffset(int32_t newZeroOffset)
{
  ...
  Serial.print("{\"type\":\"debug\",\"msg\":\"New zero: ");
  Serial.print(String(_zeroOffset));
  Serial.println("\"}");
}
...
void NAU7802::calculateCalibrationFactor(float weightOnScale, uint16_t averageAmount)
{
...
{
  Serial.print("{\"type\":\"debug\",\"msg\":\"New calibration factor: ");
  Serial.print(String(newCalFactor));
  Serial.println("\"}");
...
float NAU7802::getWeight(bool allowNegativeWeights, uint16_t samplesToTake)
{
