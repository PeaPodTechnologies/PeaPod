...
{
public:
  NAU7802(TwoWire *wire = &Wire);                                               //Default constructor
  bool begin(uint8_t addr = 0x2A, bool initialize = true); //Check communication and initialize sensor
  ...
  int32_t getAverage(uint16_t samplesToTake); //Return the average of a given number of readings

  void calculateZeroOffset(uint16_t averageAmount = 8); //Also called taring. Call this with nothing on the scale
  ...
  void calculateCalibrationFactor(float weightOnScale, uint16_t averageAmount = 8); //Call this with the value of the thing on the scale. Sets the calibration factor based on the weight on scale and zero offset.
  ...
  float getWeight(bool allowNegativeWeights = false, uint16_t samplesToTake = 8); //Once you've set zero offset and cal factor, you can ask the library to do the calculations for you.
