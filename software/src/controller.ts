...
export default class MicroController implements Controller {
				...
				switch (msg.type) {
					case 'revision':
						...
						if (msg.data === CONTROLLER_REVISION) {
							...
							Spinner.fail(
								`Microcontroller software out of date! Got ${msg.data}, expected ${CONTROLLER_REVISION}.`
							);
	...
	private resetTimeout(timeoutSeconds: number = SERIAL_TIMEOUT_SECONDS): void {
		...
		this.timeout = setTimeout(() => {
			Spinner.fail(
				`Microcontroller serial communications timed out after ${timeoutSeconds} seconds.`
			);
			this.reset();
		}, timeoutSeconds * 1000);
	}
	...
	async start(onMessage: (msg: ControllerMessage) => any): Promise<void> {
		for (const label of Object.keys(this.parameters)) {
			this.intervals.push(
				setInterval(() => {
					onMessage(
						this.generateData(label, this.parameters[label].min, this.parameters[label].max)
					);
