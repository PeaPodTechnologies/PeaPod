...
export type Publisher = {
	start(
		onConfig: (message: PubSubConfig) => void,
		onCommand: (message: PubSubCommand) => void
	): Promise<{ projectid: string; projectname?: string; run: string }>;
	stop(): void;
...
export default class PubSubPublisher implements Publisher {
	...
	publish(msg: PubSubMessage): void {
		...
		const topic =
			...
			(msg.type === 'data' ? 'events/data' : msg.type === 'info' ? 'events' : 'state');
	...
	private async selectProject(): Promise<[DocumentReference, string]> {
		const myProjects = query(
			...
			where('owners', 'array-contains', getAuth().currentUser?.uid)
		);
		...
		const ref = (
			await inquirer.prompt<{ ref: [DocumentReference, string] }>([
				{
					type: 'list',
					name: 'ref',
	 ...
	 */
	private async getDeviceInfoOrRegister(
		user: string
	): Promise<{ privatekey: string; deviceid: string }> {
		if (fs.existsSync('./rsa_private.pem') && fs.existsSync('./deviceInfo.json')) {
			...
			let deviceinfo: { id: string; owner: string } = JSON.parse(
				fs.readFileSync('./deviceInfo.json').toString()
			);
			...
			const registerDevice = httpsCallable<void, RegisterResponse>(
				getFunctions(),
				'registerDevice'
			);
			...
			fs.writeFileSync(
				'./deviceInfo.json',
				JSON.stringify(
					{ name: result.name, id: result.id, owner: getAuth().currentUser?.uid },
					null,
					2
				)
			);
...
export class OfflinePublisher implements Publisher {
	async start(
		...
		Spinner.info(
			`Logging data to ${chalk.bold('projects/' + config.projectid + '/runs/' + config.run + '/')}`
		);
		return config;
	}
	stop() {}
			...
			case 'data':
				for (const label of Object.keys(msg.data)) {
					for (const datum of msg.data[label]) {
						console.log(
							`[${chalk.magenta(msg.type.toUpperCase())}] - [${new Date(
								datum.timestamp
							).toLocaleTimeString()}] - ${label}: ${datum.value}`
						);
					}
					...
					writeFileSync(
						`${dir}${label + '-' + uuid() + '.json'}`,
						JSON.stringify(msg.data[label], null, 2)
					);
				}
				break;
			default:
