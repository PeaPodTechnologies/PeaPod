...
\begin{tabular}{| @{\makebox[2.4em][c]{\metricrow}} | p{9.45cm} | p{5.1cm} |} 
    ...
    % \hline
    Reharvest Period - Fruiting Plants \mlabel{m:time_reharvest} \hfill (\ref{ll:time_harvest}) & days (each plant)  \\
    \hline
    Germination Time \mlabel{m:time_germination} \hfill (\ref{ll:time_germination}) & hours (each plant) \\
    \hline
    Time to Harvest \mlabel{m:time_harvest} \hfill (\ref{ll:time_growth}) & days from planting (each plant) \\
    \hline
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\textwidth,angle=90,origin=c]{images/requirements-graph.png}
