...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \begin{enumerate}
            ...
            \begin{itemize}
                \item \textit{Option 1} (for 2-4 unit housing combinations): Operate the combined housing off \textbf{one} control module.
                \item \textit{Option 2} (for 5+ unit housing combinations): Add control modules to account for additional air volume, plant count, power requirement, etc. and operate in a \textbf{master-slaves topology}.
                \item \textit{Option 3} (no combined units, frame connection only): Leave the dividing panel, add a control module, and operate the two PeaPods \textbf{separately}.
            \end{itemize}
        \end{enumerate}
    \end{enumerate}
    \item \textit{Shutdown}:
