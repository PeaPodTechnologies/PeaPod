...
\newpage
    ...
    \begin{itemize}
        ...
        \item etc.
    \end{itemize}
    \item \textit{Environment Sensors}: Record the environment's current state. One for each environment control loop (\ref{sec:environment}), as well as CO${}_2$ ppm.
    ...
    \item \textit{Actuators}: Induce a change. Covers each environment control (\ref{sec:environment}, \ref{sec:lighting});
\end{itemize}
