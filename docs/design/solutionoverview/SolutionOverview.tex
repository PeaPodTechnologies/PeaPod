...
\begin{itemize} 
    ...
    \item Water is heated or cooled to a given temp (\ref{sec:program}); % TODO: Add water temp to environment control?
...
\textbf{Function}:
\begin{itemize}
    \item \textbf{Inputs}: Pressurized RO water, per-solution flow-ratio control signal (calculated from desired per-nutrient concentrations; \ref{sec:program}), pH flow-ratio control signal (calculated from desired pH; \ref{sec:program})
