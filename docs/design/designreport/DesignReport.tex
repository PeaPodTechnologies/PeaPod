...
A major innovation of PeaPod is its range and accuracy of environment generation.

Compared to large-scale vertical farming, PeaPod’s isolation and "parameter independence" makes it possible to simulate many climates. Wide LED spectrum has the potential to emit both near-infrared and near-ultraviolet light, which is important in triggering hormonal responses and creating certain compounds in plants. Combined with effective insulation, self-contained humidification and dehumidification, and powerful yet precise thermoelectric heating and cooling, PeaPod is able to generate extreme environments and even simulate conditions on other planets (minus the gravity and atmospheric changes). 

These parameters are also independent: for example, when high lighting power generates heat, thermoregulation cooling maintains the set environment. Additionally, inline nutrient and pH solution injection results in more accurate dosage. This eliminates drawbacks of reservoir/tank use, such as using less space, less solution mixing time, and avoiding a "control loop" of pH or nutrients.

%These innovations allow PeaPod to grow virtually any plant. 

\textbf{Form Factor and Extendability}

The wide range of output environments is also possible because of the small form factor of a single PeaPod unit. Warehouse-scale vertical farming solutions cannot provide fast wide-spectrum control due to the size of the operation, for example they cannot maintain extreme temperatures due to poor building insulation and uneven air circulation. PeaPod solves both of these issues with a small, modular design that allows for robust lighting, heating, and cooling systems, both at home and at scale.

PeaPod’s modularity is good for smaller facilities. When volume or dimension is a constraint, PeaPod can be reformed to fit its surroundings.

Space and time savings are a major benefit of this design feature, as the equivalent output of an entire farming or hydroponics operation (requiring a flat field or warehouse) can instead be spread throughout unused space via smaller PeaPod setups. This means a large yield can be generated without construction, zoning, labour, or any of the logistical issues that accompany large farming or hydroponic setups.

The macroscopic modularity of PeaPod maximizes available space (corners, under shelves, on a desk) while its internal modularity lets users grow any crop with only a quick snap of a connector; this contrasts large operations which are purpose-built for a single crop. Where existing systems have massive yields given a large area and permanent construction, PeaPod can grow any plant given a spot as small as half a meter on each side, comparable to a larger 3D printer.

\textbf{Optimization}

Existing solutions lack output-based optimization and research capabilities. PeaPod solves this by monitoring both internal environment and plant yield quality and health indicators. This allows for repeatable and controlled trials with scientific validity.

Once a plant's phenology is "digitized", PeaPod counteracts declining health/quality indicators in real time, saving crops. As it repeats trials and analyzes data, it generates growth plans tailored to each species and target output metric, maximizing any aspect. This makes it a valuable tool for growing and regrowing crops, as it has a large pool of data from which to derive growth plans, conditions, health indicators, and more.

\textbf{Open-Source Design and Data}

Finally, since PeaPod is open-source and made of standard parts, units can be sourced in bulk at reduced price and assembled by anybody. Public, open-source contribution to the project (both in design improvements and data collection) increases the reliability and safety of the solution. In addition, collected data is, ideally, committed to the public domain, meaning that anybody with a PeaPod can run the same iteration with the same program and species to boost scientific validity, or run a different program to expand PeaPod's knowledge base.

\uline{\textbf{3938 Characters} (938 over)}
