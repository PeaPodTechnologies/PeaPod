...
\begin{enumerate}
    \item Determine PeaPod modularity configuration from from required environment diversity from desired crop output;
    \item Assemble housing;
    \item Install control module(s):
    \begin{enumerate}
        \item Hook up water, power, and network inputs;
        \item Fill nutrient and pH adjustment solution containers;
        \item 
    \end{enumerate}
    \item Assemble trays, for each:
    \begin{enumerate}
        \item Mount lighting system, hook up power and signal to CM; \textbf{OR}
        \item Mount grow system, hook up water delivery line to CM;
    \end{enumerate}
    \item Prepare and plant seeds for desired crop output, seal growth environment;
    \item Enable primary power supply, and power on automation system, allow to perform self-test and calibrations:
    \item Open water input shutoff valve;
    \item Input program for required environments and activate;
\end{enumerate}

\textit{Food Production Cycle}:

\begin{enumerate}
    \item Plants are grown by PeaPod, and environment is observed live at a computer terminal via sensor data and camera feed;
    \item Perform maintenance, including:
    \begin{itemize}
        \item Cleaning nozzle once a month;
        \item Recharging dehumidification cartridge when instructed;
        \item Respond to all other automated notifications as instructed;
    \end{itemize}
...
% Include each of the following:
    % iso standard ($200 tho) https://www.iso.org/obp/ui/#iso:std:iso:9241:-210:en

    % Operational footprint (how much space is needed for the solution and its related processes?)
    % 16 peapods - 0.5 meters cubed each - 2m cubed total

    % Setup steps
    % include assembling the peapods

    % Food production cycle
    % highly dependent on which plants are selected

    % Food handling 
    % harvesting - specific per plant
    % storage is covered by the peapod
    % cooking time will vary based on the vegetables selected.

    % Cleaning and stowage procedures of equipment
    % extra parts can be sanitized and stowed in a compact, collapsed arragement.
    % peapods can be easily taken apart and reasembled so each component can be individually sanitized if needed.

    % Time needed for the crew to operate the solution
    % peapods can be assembled in less than 30 minutes - bulk of time come from harvesting and planting (plant specific)
