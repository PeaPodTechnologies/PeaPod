...
%OpenComment [ ]GET 2nd opinion on 4 sections - NV
...
\subsubsection{Palatability of Crop Output} %>= 6.0
...
\subsubsection{Air Temperature Control Range} %Min < 15 degC, Max > 30 degC


\subsubsection{Air Humidity Control Range} %Min < 20 %RH, Max > 90 %RH

\subsubsection{Light Spectrum Wavelength Range} %Min < 300 nm (Near-UV), Max > 800 nm (Near-IR)

\subsubsection{Light Spectrum PAR Match} %>= 95% match

\subsubsection{Light Intensity Control Range} %Min = 0, Max >= typical horitculture

\subsubsection{Internal Circulation Airflow Control Range} %Min = 0 m^3/min, Max >= 2 m^3/min

\subsubsection{Nutrient Solution Delivery Control Range} %Min = 0, Max >= max plant requirement

\subsubsection{Nutrient Solution Temp. Control Range} %Min < 10 degC, Max > 25 degC

\subsubsection{Nutrient Concentrations Control Range} %Min = 0, Max >= max plant requirement

\subsubsection{Time Requirement - Maintenance} %4 hrs/week

\subsubsection{Loss of Functionality Over 3 Years} %<= 10%

\subsubsection{Input Lifetime while Safe, Useful} %>= 3 years

\subsubsection{Outer Dimensions} %Fits through 1.07m x 1.90m doorway; W < 1.820m, D < 2.438m, H < 2.591m

\subsubsection{Outer Volume} %<= 2m^3

\subsubsection{Power Consumption} %Avg <1500 W, Peak < 3000 W
...
\subsubsection{Customer-facing Food Service} %OpenComment does this make sense? feels a little weak/unfounded - NV
...
\subsubsection{Crowdsourced Research} %ClosedComment feel better about this one, almost like a better version of the above ^ - NV
...
\subsubsection{De-centralized Production} %ClosedComment I like this one, should add more though - NV
