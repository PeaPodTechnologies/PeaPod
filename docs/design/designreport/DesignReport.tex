...
PeaPod is an automated plant growth environment, made of control systems and an automation/monitoring system within modular, cubic housing. It is able to generate any desired environment while collecting data on plant growth and improving yields over time.

NOTE: For this submission's constraints, only the \textbf{Standard Extended PeaPod} is in consideration: a 4x3 unit PeaPod "wall," with one control module and one of each tray (grow and lighting) per unit.
...
\begin{itemize}
    ...
    \item \textit{Aeroponics}: Reverse osmosis (RO) water is pressurized by a pump, brought to temperature by a thermoelectric water block, fed nutrient and pH solutions by parallel Venturis, and forced through a solenoid to nozzles mounted beneath the plant support trays. Temperature and pressure sensors providing Proportionate-Integral-Derivative (PID) feedback maintain safety cutoffs. Injection rates for nutrients and pH are controlled by servo-actuated flow-control valves, while runoff water is pumped from the container basin and recycled to the injection manifold. Both runoff and supply lines are quick-disconnect for tray removal during planting and harvest. Aeroponics is chosen for water efficiency (98\% less than farming), nutrient efficiency (60\% less), no pH/nutrient leakback (common in reservoir-based hydroponics), increased root oxygenation, and greater supported crop variety. % https://www.nasa.gov/vision/earth/technologies/aeroponic_plants.html
    \item \textit{Air Thermoregulation}: Leaf zone air temperature is regulated by a thermoelectric heat pump. Thermoelectric/Peltier tiles heat or cool a heat sink (via thermal compound). Fans blow air over the heat sinks to distribute heat to homogenously heat or cool the environment. A matching heat sink and fan on the opposite face of the Peltier tiles complete the pump by conducting heat to/from surroundings. A PID control system is informed by temperature sensors throughout the growth environment, and---with the use of a MOSFET H-bridge and dimmable current source---controls the direction and magnitude of the heat transfer. This method is chosen for its low complexity (no liquids, pressurization, etc.) and ease of automation (bidirectional heat pump, precisely dimmable, PID tuning for high accuracy).
    ...
    \begin{itemize}
        \item \textit{Humidification}: Humidity is \textbf{increased} by an ultrasonic mesh nebulizer. RO water is supplied to a container with a fine mesh piezoelectric disc. A control signal activates a driver circuit which causes the disc to oscillate, producing water vapour. This method is chosen for its ease of automation and consistency of vapour production.
        \item \textit{Dehumidification}: Humidity is \textbf{decreased} by a rechargeable desiccant cartridge. The cartridge of dry silica gel beads is covered by servo-actuated "shutters" that prevent unintended air movement through the cartridge. Fans draw humid air through a HEPA filter into the cartridge, expelling dry air back into the growth environment. The beads change color to indicate water saturation, which is sensed by the automation controller. The crew is then notified to swap the saturated cartridge for a dry one, and to "recharge" the saturated cartridge via evaporation in a standard oven.
    \end{itemize}
...
\uline{\textbf{5029 Characters} (2029 over)}
