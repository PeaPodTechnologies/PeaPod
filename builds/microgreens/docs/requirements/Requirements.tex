...
% Headers
\pagestyle{fancy}
\lhead{PeaPod for Microgreens - Requirements}
...
\begin{titlepage}
    \begin{center}
        ...
        \textbf{\large{PeaPod for Microgreens - Requirements}}

        \vspace{0.5cm}

        Outlining the Implementation-Specific Requirements for a PeaPod Microgreens Production Unit\\
...
The purpose of this document is to outline both the category requirements (Section \ref{sec:requirements}) for an implementation of the PeaPod framework (See \textit{PeaPod - Requirements}) that produces microgreens and the scoped requirements (Section \ref{sec:scope}) for this design as proposed by PeaPod Technologies Inc.: \textbf{PeaPod for Microgreens}.
